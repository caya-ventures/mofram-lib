...
module {
  ...
  public func dateTimeToTimestamp(dateTime : DateTime) : Int {
    ...
    days += daysFromYearStart(checkLeapYear(dateTime.year),dateTime.month);
    days += dateTime.day-1;

    return timeToTime(dateTime) + daysToTime(days);
  };
  ...
  public func timeToTime (time: DateTime): Time.Time {
    ((time.hour * 3600) + (time.min * 60) + time.sec) * 1000000000;
  };
  public func daysToTime (days: Int): Time.Time {
    days * 24 * 3600 * 1000000000;
  };
  public func timestampToDateTime (msTimestamp: Time.Time) : DateTime { 
    let timestamp : Int = msTimestamp/1000000000;
